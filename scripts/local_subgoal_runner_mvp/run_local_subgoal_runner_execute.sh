...
if ! ${TARGET_REGEN_CMD}; then
  echo '{"final_decision":"EXECUTE_BLOCKED_BY_TARGET_REGENERATION_FAILURE","target_regeneration_pass":false}'
  exit 1
fi

FRAME_CONTRACT_CMD="python3 scripts/local_subgoal_runner_mvp/frame_contract_validation.py"
if ! ${FRAME_CONTRACT_CMD}; then
  echo '{"final_decision":"EXECUTE_BLOCKED_BY_TARGET_REGENERATION_FAILURE","target_regeneration_pass":true,"frame_contract_validation_pass":false}'
  exit 1
fi
...
python3 scripts/local_subgoal_runner_mvp/local_subgoal_runner.py \
  --execute \
  --simple-motion-profile true \
  --heading-threshold-rad 0.35 \
  --align-max-steps 20 \
  --align-max-runtime-sec 180 \
  --angular-z-max 0.30 \
  --turn-slice-sec 5.00 \
  --linear-x 0.30 \
  --forward-slice-sec 5.00 \
  --grid-sample-timeout-sec 8.0 \
  --local-goal-radius-m 3.00 \
  --max-runtime-sec 300 \
  --max-steps 80 \
  "$@"
