...
class LivoxPointsPlugin : public RayPlugin {
 ...
 private:
    ...
    // Diagnostic-only counters. They neither alter the scan pattern nor the
    // publication contract; startup audit consumes their throttled gzserver log.
    uint64_t loadEnterCount = 0;
    uint64_t parentSensorCastOk = 0;
    uint64_t sensorActiveBefore = 0;
    uint64_t sensorActiveAfter = 0;
    uint64_t callbackConnectionOk = 0;
    uint64_t callbackInvocationCount = 0;
    uint64_t measurementTimeAdvanceCount = 0;
    uint64_t rayCountDiagnostic = 0;
    uint64_t validPointCount = 0;
    uint64_t publishAttemptCount = 0;
    uint64_t publishSuccessCount = 0;
    gazebo::common::Time lastDiagnosticMeasurementTime;
