...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
    ++loadEnterCount;
    ROS_INFO_STREAM("[LIVOX_DIAG] load_enter_count=" << loadEnterCount);
    ...
    ROS_INFO_STREAM("[LIVOX_DIAG] csv_read_ok rows=" << datas.size());


    
    sdfPtr = sdf;
    ROS_INFO_STREAM("[LIVOX_DIAG] before_plugin_ray_config");
    auto rayElem = sdfPtr->GetElement("ray");
    if (!rayElem) {
        gzerr << "[LIVOX_DIAG] missing_plugin_ray_config; stopping Load safely\n";
        return;
    }
    auto scanElem = rayElem->GetElement("scan");
    auto rangeElem = rayElem->GetElement("range");
    if (!scanElem || !rangeElem) {
        gzerr << "[LIVOX_DIAG] missing_plugin_scan_or_range_config; stopping Load safely\n";
        return;
    }
    ...
    raySensor = _parent;
    auto typedRaySensor = std::dynamic_pointer_cast<gazebo::sensors::RaySensor>(_parent);
    if (!typedRaySensor) {
        gzerr << "[LIVOX_DIAG] parent_sensor_cast_ok=0; stopping Load safely\n";
        return;
    }
    ++parentSensorCastOk;
    sensorActiveBefore = typedRaySensor->IsActive() ? 1 : 0;
    ROS_INFO_STREAM("[LIVOX_DIAG] parent_sensor_cast_ok=" << parentSensorCastOk
                    << " sensor_active_before=" << sensorActiveBefore);
    ...
    typedRaySensor->SetActive(true);
    sensorActiveAfter = typedRaySensor->IsActive() ? 1 : 0;
    callbackConnectionOk = sensorActiveAfter ? 1 : 0;
    ROS_INFO_STREAM("[LIVOX_DIAG] base_ray_plugin_load_returned=1 sensor_active_after="
                    << sensorActiveAfter << " callback_connection_ok=" << callbackConnectionOk);
    ...
    rayShape->Init();
    ROS_INFO_STREAM("[LIVOX_DIAG] custom_ray_shape_ready=1");
    ...
    ROS_INFO_STREAM("[LIVOX_DIAG] load_complete rays_configured=" << (samplesStep / downSample));
}

void LivoxPointsPlugin::OnNewLaserScans() {
    ++callbackInvocationCount;
    const auto now = world->SimTime();
    if (now > lastDiagnosticMeasurementTime) {
        ++measurementTimeAdvanceCount;
        lastDiagnosticMeasurementTime = now;
    }
    if (rayShape) {
        ...
        rayCountDiagnostic = points_pair.size();
        ...
        uint64_t validPointsThisCallback = 0;
        ...
        for (auto &pair : points_pair) {
            ...
            //   auto index = (verticalRayCount - verticle_index - 1) * rayCount + horizon_index;
                ...
                // Rays start at minDist; ODE depth is measured from that shifted
                // origin, so restore minDist once before publishing sensor-origin XYZ.
                range += minDist;
        ...
        validPointCount += validPointsThisCallback;
        if (scanPub && scanPub->HasConnections()) scanPub->Publish(laserMsg);
        ++publishAttemptCount;
        rosPointPub.publish(scan_point);
        ++publishSuccessCount;
        if (callbackInvocationCount == 1 || callbackInvocationCount % 10 == 0) {
            ROS_INFO_STREAM("[LIVOX_DIAG] callbacks=" << callbackInvocationCount
                            << " measurement_time_advance_count=" << measurementTimeAdvanceCount
                            << " ray_count=" << rayCountDiagnostic
                            << " valid_point_count=" << validPointCount
                            << " publish_attempt_count=" << publishAttemptCount
                            << " publish_success_count=" << publishSuccessCount
                            << " ros_subscribers=" << rosPointPub.getNumSubscribers());
        }
        ros::spinOnce();
    }
}
