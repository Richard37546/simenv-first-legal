...
| 你要做什么 | 推荐阅读 |
|------------|----------|
| 第一次启动环境 | [快速启动](quick-start.md) |
| 接入导航、感知或控制算法 | [算法接入接口](algorithm-interfaces.md) |
| 理解楼栋、危险源和干扰源 | [比赛场景规则](competition-rules.md) |
| 控制门和电梯 | [门与电梯控制](doors-and-elevator.md) |
| 输出结果并计算分数 | [结果格式与评估方法](evaluation.md) |
| 查看传感器安装、话题和坐标系 | [传感器与 ROS 话题](sensors-and-topics.md) |
| 处理启动 warning 或服务异常 | [常见问题](troubleshooting.md) |
| 查看旧版完整长文档 | [完整参考文档](reference.md) |
...
`auto.sh` 会自动完成随机场景生成、Gazebo 启动、A1 模型与传感器启动、门/电梯控制服务启动和 `junior_ctrl` 控制器启动。更多启动方式见 [快速启动](quick-start.md)。
...
`junior_ctrl` 默认以前台方式启动。终端输入 `2` 进入站立状态，输入 `6` 切换到 RL 模式，随后控制器接收 `/cmd_vel`。完整接口见 [算法接入接口](algorithm-interfaces.md)。
...
评分细则和匹配规则见 [结果格式与评估方法](evaluation.md)。
